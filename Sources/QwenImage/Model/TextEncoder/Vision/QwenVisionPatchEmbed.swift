...
final class QwenVisionPatchEmbed: Module {
  ...
  func callAsFunction(_ patches: MLXArray) -> MLXArray {
    ...
    QwenLogger.vision.debug("weight shape=\(projection.weight.shape) kernel reshaped=\(kernel.shape)")
    let transposed = kernel.transposed(1, 0)
    let inputs = patches
    let projected = MLX.matmul(inputs, transposed)
    let final = projected.asType(targetType)
    QwenLogger.vision.debug("output shape=\(final.shape)")
    return final
  }
}
